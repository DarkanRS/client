public class Class282_Sub17_Sub2 extends Class282_Sub17 {

	static FontMetrics aClass414_9933;
	public static boolean[] VARC_BOOL;
	int anInt9932;
	Class348 this$0;

	void method12252(ClanSettings class61_1) {
		class61_1.method1233(-518396991 * this.anInt9932 * 390605889, -1611815264);
	}

	void method12250(RsByteBuffer rsbytebuffer_1) {
		this.anInt9932 = rsbytebuffer_1.readUnsignedShort();
	}

	void method12257(RsByteBuffer rsbytebuffer_1) {
		this.anInt9932 = rsbytebuffer_1.readUnsignedShort() * 390605889 * -518396991;
	}

	void method12249(RsByteBuffer rsbytebuffer_1) {
		this.anInt9932 = rsbytebuffer_1.readUnsignedShort() * 390605889 * -518396991;
	}

	Class282_Sub17_Sub2(Class348 class348_1) {
		this.this$0 = class348_1;
		this.anInt9932 = -1;
	}

	void method12253(RsByteBuffer rsbytebuffer_1) {
		this.anInt9932 = rsbytebuffer_1.readUnsignedShort() * 390605889 * -518396991;
	}

	void method12254(ClanSettings class61_1) {
		class61_1.method1233(-518396991 * this.anInt9932 * 390605889, -1676142453);
	}

	void method12255(ClanSettings class61_1) {
		class61_1.method1233(-518396991 * this.anInt9932 * 390605889, -1518370780);
	}

	void method12256(ClanSettings class61_1) {
		class61_1.method1233(-518396991 * this.anInt9932 * 390605889, -1225756314);
	}

	void method12258(ClanSettings class61_1) {
		class61_1.method1233(-518396991 * this.anInt9932 * 390605889, -1307878943);
	}

	void method12251(ClanSettings class61_1) {
		class61_1.method1233(this.anInt9932, -1765056942);
	}

}
