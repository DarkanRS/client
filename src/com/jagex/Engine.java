...
public abstract class Engine implements Interface24, Runnable, FocusListener, WindowListener {
    ...
    boolean method4665() {
        String string_2 = SubInterface.suppliedApplet.getDocumentBase().getHost().toLowerCase();
        if (!"jagex.com".equals(string_2) && !string_2.endsWith(".jagex.com")) {
            if (!"runescape.com".equals(string_2) && !string_2.endsWith(".runescape.com")) {
                if (!"stellardawn.com".equals(string_2) && !string_2.endsWith(".stellardawn.com")) {
                    if (!string_2.endsWith("127.0.0.1") && !string_2.endsWith("trentonkress.com")) {
                        while (!string_2.isEmpty() && string_2.charAt(string_2.length() - 1) >= 48 && string_2.charAt(string_2.length() - 1) <= 57) {
                            string_2 = string_2.substring(0, string_2.length() - 1);
                        }
                        if (string_2.endsWith("192.168.1.")) {
                            return true;
                        } else {
                            method4680("invalidhost");
                            return false;
                        }
                    } else {
                        return true;
