...
public class HitbarDefinitions {
	...
	static void method3230(int i_0, int i_1, int i_2, int i_3, int i_4, int i_5, int i_6) {
		...
		for (i_10 = i_2; i_10 < i_8; i_10++)
			KeyHoldInputSubscriber.method3922(Class532.anIntArrayArray7072[i_10], i_0, i_1, i_5);
		for (i_10 = i_3; i_10 > i_9; --i_10)
			KeyHoldInputSubscriber.method3922(Class532.anIntArrayArray7072[i_10], i_0, i_1, i_5);
		i_10 = i_0 + i_6;
		int i_11 = i_1 - i_6;
		for (int i_12 = i_8; i_12 <= i_9; i_12++) {
			int[] ints_13 = Class532.anIntArrayArray7072[i_12];
			...
			if (bool_7)
				...
				if (Camera.STATE == CamState.FOLLOW_PLAYER) {
					...
					if (Camera.anInt7273 >> 8 > i_8)
						...
						if (Camera.SHAKING[4] && Camera.SHAKE_2[4] + 128 > i_8)
				...
				int i_11 = Camera.anInt3512;
				int i_12 = Camera.anInt5930;
				int i_14;
				for (int slotId = 0; slotId < 5; slotId++)
					if (Camera.SHAKING[slotId]) {
						i_14 = (int) (Math.random() * (Camera.SHAKE_1[slotId] * 2 + 1) - Camera.SHAKE_1[slotId] + Math.sin(Camera.SHAKE_4[slotId] * (Camera.SHAKE_3[slotId] / 100.0D)) * Camera.SHAKE_2[slotId]);
						if (slotId == 0)
							Camera.CAM_MOVE_ABSOLUTEX += i_14 << 2;
						if (slotId == 1)
							Camera.CAM_MOVE_ABSOLUTEZ += i_14 << 2;
						if (slotId == 2)
							Camera.CAM_MOVE_ABSOLUTEY += i_14 << 2;
						if (slotId == 3)
							Camera.anInt5930 = i_14 + Camera.anInt5930 & 0x3fff;
						if (slotId == 4) {
							Camera.anInt3512 += i_14;
							if (Camera.anInt3512 < 1024)
								Camera.anInt3512 = 1024;
							else if (Camera.anInt3512 > 3072)
								Camera.anInt3512 = 3072;
						}
					}
				if (Camera.CAM_MOVE_ABSOLUTEX < 0)
					Camera.CAM_MOVE_ABSOLUTEX = 0;
				if (Camera.CAM_MOVE_ABSOLUTEX > (IndexLoaders.MAP_REGION_DECODER.getSceneObjectManager().sizeX << 9) - 1)
					Camera.CAM_MOVE_ABSOLUTEX = (IndexLoaders.MAP_REGION_DECODER.getSceneObjectManager().sizeX << 9) - 1;
				if (Camera.CAM_MOVE_ABSOLUTEY < 0)
					Camera.CAM_MOVE_ABSOLUTEY = 0;
				if (Camera.CAM_MOVE_ABSOLUTEY > (IndexLoaders.MAP_REGION_DECODER.getSceneObjectManager().sizeY << 9) - 1)
					Camera.CAM_MOVE_ABSOLUTEY = (IndexLoaders.MAP_REGION_DECODER.getSceneObjectManager().sizeY << 9) - 1;
				...
				client.aClass294_7457.method5223((-Camera.CAM_MOVE_ABSOLUTEX), (-Camera.CAM_MOVE_ABSOLUTEZ), (-Camera.CAM_MOVE_ABSOLUTEY));
				client.aClass294_7457.rotation(0.0F, -1.0f, 0.0F, Trig.degToRad(-Camera.anInt5930 & 0x3fff));
				client.aClass294_7457.rotation(-1.0f, 0.0F, 0.0F, Trig.degToRad(-Camera.anInt3512 & 0x3fff));
				client.aClass294_7457.rotation(0.0F, 0.0F, -1.0f, Trig.degToRad(-Camera.anInt1525 & 0x3fff));
				...
				if (class535_16.method11451() != null) {
					...
					class535_16.method11451().method4215(Renderers.CURRENT_RENDERER, Class388.anInt4723 << 3, i_01, i_13, i_21, i_31, Camera.anInt3512, Camera.anInt5930, Camera.anInt1525, i_14);
				} else
				...
				IndexLoaders.MAP_REGION_DECODER.getSceneObjectManager().method3447(client.FRAME_COUNT, Camera.CAM_MOVE_ABSOLUTEX, Camera.CAM_MOVE_ABSOLUTEZ, Camera.CAM_MOVE_ABSOLUTEY, IndexLoaders.MAP_REGION_DECODER.method4532(), client.anIntArray7198, client.anIntArray7303, client.anIntArray7194, client.anIntArray7305, client.anIntArray7306, VertexNormal.MY_PLAYER.plane + 1, b_15, Class393.preferences.flickeringEffects.method12943() == 0);
				++client.anInt7396;
				...
				Camera.CAM_MOVE_ABSOLUTEX = i_8;
				Camera.CAM_MOVE_ABSOLUTEZ = i_9;
				Camera.CAM_MOVE_ABSOLUTEY = i_10;
				Camera.anInt3512 = i_11;
				Camera.anInt5930 = i_12;
