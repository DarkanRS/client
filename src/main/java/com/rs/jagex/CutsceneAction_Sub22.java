		...
		if (Class20.anInt169 < 412)
			if (VertexNormal.MY_PLAYER == target) {
				if (client.IS_USE_SELECTED && (Class506.USE_OPTIONS_FLAGS & 0x10) != 0)
					PlayerModel.method4032(client.aString7275, client.aString7356 + " " + "->" + " " + Utils.rgbToColHexShortcut(16777215) + LocalizedText.aClass433_5299.translate(Class223.CURRENT_LANGUAGE), Defaults8Loader.anInt5932, MenuAction.TGT_SELF, -1, 0L, 0, 0, true, false, target.index, false);
			} else {
				String string_3;
				if (target.elo == 0) {
				...
				if (client.IS_USE_SELECTED && !bool_1 && (Class506.USE_OPTIONS_FLAGS & 0x8) != 0)
					PlayerModel.method4032(client.aString7275, client.aString7356 + " " + "->" + " " + Utils.rgbToColHexShortcut(16777215) + string_3, Defaults8Loader.anInt5932, MenuAction.TGT_PLAYER, -1, target.index, 0, 0, true, false, target.index, false);
				if (bool_1)
					PlayerModel.method4032(Utils.rgbToColHexShortcut(13421772) + string_3, "", -1, null, 0, 0L, 0, 0, false, true, target.index, false);
				else
					for (int i_10 = 7; i_10 >= 0; --i_10)
						if (client.PLAYER_OPTIONS[i_10] != null) {
							boolean reduceAttackPriority = false;
							if (Game.darkan == client.CURRENT_GAME && client.PLAYER_OPTIONS[i_10].equalsIgnoreCase(LocalizedText.ATTACK.translate(Class223.CURRENT_LANGUAGE))) {
								...
								if (VertexNormal.MY_PLAYER.teamId != 0 && target.teamId != 0) {
									if (target.teamId == VertexNormal.MY_PLAYER.teamId)
										reduceAttackPriority = true;
								} else if (target.reducedAttackPriority)
									reduceAttackPriority = true;
							} else if (client.PLAYER_OPTION_REDUCED_PRIORITY[i_10])
								reduceAttackPriority = true;
							MenuAction action = reduceAttackPriority ? client.aShortArray7239[i_10].makeLowPriority() : client.aShortArray7239[i_10];
							int i_7 = client.PLAYER_OPTION_CURSORS[i_10] != -1 ? client.PLAYER_OPTION_CURSORS[i_10] : client.anInt7342;
							PlayerModel.method4032(client.PLAYER_OPTIONS[i_10], Utils.rgbToColHexShortcut(16777215) + string_3, i_7, action, -1, target.index, 0, 0, true, false, target.index, false);
						}
				if (!bool_1)
					for (MenuActionEvent class282_sub50_sub7_9 = (MenuActionEvent) Class20.aClass482_171.head(); class282_sub50_sub7_9 != null; class282_sub50_sub7_9 = (MenuActionEvent) Class20.aClass482_171.next())
						if (class282_sub50_sub7_9.menuAction == MenuAction.WALK) {
