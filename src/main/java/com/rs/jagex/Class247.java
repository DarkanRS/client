...
public class Class247 {
	...
	public static boolean method4248(int i_0, int i_1) {
		...
		if (Interface.INTERFACES[i_3] != null && Interface.INTERFACES[i_3].components[i_4] != null) {
			...
			MenuActionEvent event;
			if (i_1 == -1 && icomponentdefinitions_5.type == ComponentType.CONTAINER) {
				for (event = (MenuActionEvent) Class20.aClass482_171.head(); event != null; event = (MenuActionEvent) Class20.aClass482_171.next())
					if (event.menuAction == MenuAction.TGT_BUTTON || event.menuAction == MenuAction.IF_BUTTONX2 || event.menuAction == MenuAction.SELECT_TGT || event.menuAction == MenuAction.IF_BUTTONX1 || event.menuAction == MenuAction.IF_CONTINUE)
						for (IComponentDefinitions icomponentdefinitions_7 = IComponentDefinitions.getDefs(event.anInt9582); icomponentdefinitions_7 != null; icomponentdefinitions_7 = IComponentDefinitions.getParentLayer(Interface.INTERFACES[icomponentdefinitions_7.idHash >> 16], icomponentdefinitions_7))
							if (icomponentdefinitions_7.idHash == icomponentdefinitions_5.idHash)
								return true;
			} else
				for (event = (MenuActionEvent) Class20.aClass482_171.head(); event != null; event = (MenuActionEvent) Class20.aClass482_171.next())
					if (event.anInt9581 == i_1 && icomponentdefinitions_5.idHash == event.anInt9582 && (event.menuAction == MenuAction.TGT_BUTTON || event.menuAction == MenuAction.IF_BUTTONX2 || event.menuAction == MenuAction.SELECT_TGT || event.menuAction == MenuAction.IF_BUTTONX1 || event.menuAction == MenuAction.IF_CONTINUE))
						return true;
		}
		return false;
	}
	...
	public boolean method4217(AbstractRenderer graphicalrenderer_1, int i_2, int i_3) {
		if (i_2 != anInt3032) {
			...
			if (aClass334Array3049 != null) {
				...
				for (Class334 class334_8 : aClass334Array3049) {
