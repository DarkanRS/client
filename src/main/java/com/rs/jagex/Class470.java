...
public class Class470 {
	...
	public Class470() throws Exception {
		...
		if (!defaultMoniter.isFullScreenSupported()) {
			...
			for (GraphicsDevice moniter : monitersArr) {
	...
	void method7808(Frame window, int width, int height, int i_4, int i_5) {
		...
		if (i_51 == 0) {
			...
			for (DisplayMode displayMode2 : displayModes)
				if (displayMode2.getWidth() == width && displayMode2.getHeight() == height && displayMode2.getBitDepth() == i_4) {
					int i_11 = displayMode2.getRefreshRate();
	...
	void method7820() {
		if (displayMode != null) {
			...
			for (DisplayMode displaymode_6 : arr_4) {
