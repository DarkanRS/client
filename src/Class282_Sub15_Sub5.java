...
public abstract class Class282_Sub15_Sub5 extends Class282_Sub15 {
	...
	static final void decodeMasks(RsBitsBuffer buffer, int i, Player player, int flag, int i_76_) {
		...
		if ((flag & 0x20000) != 0) {
			...
			player.anInt10347 = (-1809259861 * client.cycles + buffer.readUnsignedShort128()) * -245378127;
			player.anInt10348 = (client.cycles * -1809259861 + buffer.readUnsignedShort()) * 485671023;
		}
		if ((flag & 0x400000) != 0) {
			int i_78_ = ((buffer.buffer[((buffer.index += -1115476867) * -1990677291) - 1]) & 0xff);
			for (int i_79_ = 0; i_79_ < i_78_; i_79_++) {
				int i_80_ = buffer.readShortLE((byte) -120);
		...
		if ((flag & 0x200) != 0) {
			...
			int i_83_ = buffer.readIntV2(-534504239);
			if (i_82_ == 65535)
				i_82_ = -1;
		...
		if ((flag & 0x20) != 0) {
			...
			if (1871221471 * player.anInt10355 == 0) {
				player.method15863(327043279 * player.faceDirection, 1784401370);
				player.faceDirection = -1435090479;
			}
		}
		if (0 != (flag & 0x800000)) {
			...
			for (int i_88_ = 0; i_88_ < i_87_; i_88_++) {
				int i_89_ = buffer.readShortLE((byte) -4);
				int i_90_ = buffer.readInt();
		...
		if ((flag & 0x40) != 0) {
			int numHits = buffer.readUnsigned128Byte();
			if (numHits > 0) {
				for (int hitIdx = 0; hitIdx < numHits; hitIdx++) {
					int hitSoakingMark = -1;
					int hitDamage = -1;
					int hitSoakingDamage = -1;
					int hitMark = buffer.readUnsignedSmart(1634249310);
					if (hitMark == 32767) {
						hitMark = buffer.readUnsignedSmart(1927514283);
						hitDamage = buffer.readUnsignedSmart(1968115481);
						hitSoakingMark = buffer.readUnsignedSmart(1841145813);
						hitSoakingDamage = buffer.readUnsignedSmart(2133803964);
					} else if (hitMark == 32766) {
						hitMark = -1;
						hitDamage = buffer.readUnsignedByteC(-1911985407);
					} else {
						hitDamage = buffer.readUnsignedSmart(2072861558);
					}
					int hitDelay = buffer.readUnsignedSmart(2132556063);
					player.applyHit(hitMark, hitDamage, hitSoakingMark, hitSoakingDamage, (client.cycles * -1809259861), hitDelay, 775732782);
				}
			}
			int hitbarSize = buffer.readUnsignedByte();
			if (hitbarSize > 0) {
				for (int barIdx = 0; barIdx < hitbarSize; barIdx++) {
					int barType = buffer.readUnsignedSmart(1729379803);
					int change = buffer.readUnsignedSmart(1506698812);
					if (32767 != change) {
		...
		if ((flag & 0x1) != 0) {
			int length = buffer.readUnsignedByteC(850654507);
			byte[] is = new byte[length];
			RsByteBuffer appearanceBlock = new RsByteBuffer(is);
			buffer.readBytes(is, 0, length, 1586054229);
			Class197.aClass282_Sub35Array2428[i] = appearanceBlock;
			player.decodeAppearance(appearanceBlock, 1925099603);
		}
		if ((flag & 0x10) != 0) {
		...
		if (0 != (flag & 0x2)) {
			...
			if (i_119_ == 65535)
				i_119_ = -1;
			player.faceEntity = i_119_ * -2059452093;
		}
		if (0 != (flag & 0x1000))
		...
		if (0 != (flag & 0x800)) {
			...
			if (player.aBool10568) {
				...
				player.anInt10326 += (830034083 * player.regionBaseX[0]);
				player.anInt10328 += (player.regionBaseY[0] * -2091025609);
				player.anInt10341 += (1110107435 * player.regionBaseX[0]);
				player.anInt10343 += (-1352948627 * player.regionBaseY[0]);
				player.anInt10355 = -2086688481;
			}
			player.anInt10367 = 0;
		}
		if (0 != (flag & 0x80)) {
			int i_129_ = buffer.readShortLE((byte) -74);
			int i_130_ = buffer.readInt();
			if (i_129_ == 65535)
				i_129_ = -1;
		...
		if (player.aBool10568) {
			if (movementType == 127)
				...
				if (Class249.aClass249_3083.aByte3085 != movementType)
					...
					i_134_ = Class197.playerMovementTypes[i];
