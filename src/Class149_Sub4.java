import java.util.Iterator;

public class Class149_Sub4 extends Class149 {

	int anInt9391;
	int anInt9393;
	int anInt9392;
	int anInt9394;

	void method2558(int i_1, int i_2) {
		int i_3 = 252323111 * this.anInt9391 * 249887383 * i_1 >> 12;
		int i_4 = i_1 * 232719965 * this.anInt9392 * 50986485 >> 12;
		int i_5 = this.anInt9393 * -724925473 * -1685653473 * i_2 >> 12;
		int i_6 = i_2 * this.anInt9394 * 954206695 * -1856308777 >> 12;
		Class395.method6772(i_3, i_4, i_5, i_6, 1340859839 * this.anInt1743 * 440863295, -1670932530);
	}

	void method2561(int i_1, int i_2, int i_3) {
		int i_4 = this.anInt9391 * i_1 >> 12;
		int i_5 = this.anInt9392 * i_1 >> 12;
		int i_6 = i_2 * this.anInt9393 >> 12;
		int i_7 = i_2 * this.anInt9394 >> 12;
		Class105.method1805(i_4, i_5, i_6, i_7, this.anInt1741, this.anInt1742, (byte) 9);
	}

	void method2556(int i_1, int i_2, int i_3) {
		int i_4 = this.anInt9391 * i_1 >> 12;
		int i_5 = this.anInt9392 * i_1 >> 12;
		int i_6 = i_2 * this.anInt9393 >> 12;
		int i_7 = i_2 * this.anInt9394 >> 12;
		Class395.method6772(i_4, i_5, i_6, i_7, this.anInt1743, -975151690);
	}

	void method2557(int i_1, int i_2, byte b_3) {
		int i_4 = this.anInt9391 * i_1 >> 12;
		int i_5 = this.anInt9392 * i_1 >> 12;
		int i_6 = i_2 * this.anInt9393 >> 12;
		int i_7 = i_2 * this.anInt9394 >> 12;
		Ground.method6731(i_4, i_5, i_6, i_7, this.anInt1743, this.anInt1741, this.anInt1742, 1235101523);
	}

	void method2555(int i_1, int i_2) {
		int i_3 = 252323111 * this.anInt9391 * 249887383 * i_1 >> 12;
		int i_4 = this.anInt9392 * 50986485 * 232719965 * i_1 >> 12;
		int i_5 = i_2 * -1685653473 * this.anInt9393 * -724925473 >> 12;
		int i_6 = this.anInt9394 * 954206695 * -1856308777 * i_2 >> 12;
		Class105.method1805(i_3, i_4, i_5, i_6, -1525176857 * this.anInt1741 * -1378640425, this.anInt1742 * 383825605 * -1125689331, (byte) 9);
	}

	void method2559(int i_1, int i_2) {
		int i_3 = 252323111 * this.anInt9391 * 249887383 * i_1 >> 12;
		int i_4 = this.anInt9392 * 50986485 * 232719965 * i_1 >> 12;
		int i_5 = i_2 * -1685653473 * this.anInt9393 * -724925473 >> 12;
		int i_6 = this.anInt9394 * 954206695 * -1856308777 * i_2 >> 12;
		Class105.method1805(i_3, i_4, i_5, i_6, -1525176857 * this.anInt1741 * -1378640425, this.anInt1742 * 383825605 * -1125689331, (byte) 9);
	}

	void method2562(int i_1, int i_2) {
		int i_3 = 252323111 * this.anInt9391 * 249887383 * i_1 >> 12;
		int i_4 = i_1 * 232719965 * this.anInt9392 * 50986485 >> 12;
		int i_5 = i_2 * -1685653473 * this.anInt9393 * -724925473 >> 12;
		int i_6 = i_2 * this.anInt9394 * 954206695 * -1856308777 >> 12;
		Ground.method6731(i_3, i_4, i_5, i_6, 1340859839 * this.anInt1743 * 440863295, -1525176857 * this.anInt1741 * -1378640425, this.anInt1742 * 383825605 * -1125689331, 2071365081);
	}

	void method2560(int i_1, int i_2) {
		int i_3 = 252323111 * this.anInt9391 * 249887383 * i_1 >> 12;
		int i_4 = i_1 * 232719965 * this.anInt9392 * 50986485 >> 12;
		int i_5 = this.anInt9393 * -724925473 * -1685653473 * i_2 >> 12;
		int i_6 = i_2 * this.anInt9394 * 954206695 * -1856308777 >> 12;
		Class395.method6772(i_3, i_4, i_5, i_6, 1340859839 * this.anInt1743 * 440863295, -1875945632);
	}

	Class149_Sub4(int i_1, int i_2, int i_3, int i_4, int i_5, int i_6, int i_7) {
		super(i_5, i_6, i_7);
		this.anInt9391 = i_1;
		this.anInt9393 = i_2;
		this.anInt9392 = i_3;
		this.anInt9394 = i_4;
	}

	static final void method14659(CS2Executor cs2executor_0, byte b_1) {
		int i_2 = cs2executor_0.intStack[--cs2executor_0.intStackPtr];
		IComponentDefinitions icomponentdefinitions_3 = Class117.method1981(i_2, (byte) 45);
		cs2executor_0.intStack[++cs2executor_0.intStackPtr - 1] = icomponentdefinitions_3.anInt1314;
	}

	static final void method14660(CS2Executor cs2executor_0, int i_1) {
		int i_2 = cs2executor_0.intStack[--cs2executor_0.intStackPtr];
		IComponentDefinitions icomponentdefinitions_3 = Class117.method1981(i_2, (byte) 122);
		Class88.method1494(icomponentdefinitions_3, cs2executor_0, (byte) -43);
	}

	static final void method14661(byte b_0) {
		Iterator iterator_1 = client.aClass465_7334.iterator();

		while (iterator_1.hasNext()) {
			Class282_Sub50_Sub10 class282_sub50_sub10_2 = (Class282_Sub50_Sub10) iterator_1.next();
			Class521_Sub1_Sub1_Sub4 class521_sub1_sub1_sub4_3 = class282_sub50_sub10_2.aClass521_Sub1_Sub1_Sub4_9636;
			class521_sub1_sub1_sub4_3.method15926(1, (byte) 16);
			if (class521_sub1_sub1_sub4_3.method15928((short) 11184)) {
				class282_sub50_sub10_2.remove();
				class521_sub1_sub1_sub4_3.method15931(-2018214290);
			}
		}

	}

	static String method14662(int i_0, int i_1, boolean bool_2, int i_3) {
		if (i_1 >= 2 && i_1 <= 36) {
			if (bool_2 && i_0 >= 0) {
				int i_4 = 2;

				for (int i_5 = i_0 / i_1; i_5 != 0; i_4++) {
					i_5 /= i_1;
				}

				char[] arr_6 = new char[i_4];
				arr_6[0] = 43;

				for (int i_7 = i_4 - 1; i_7 > 0; --i_7) {
					int i_8 = i_0;
					i_0 /= i_1;
					int i_9 = i_8 - i_0 * i_1;
					if (i_9 >= 10) {
						arr_6[i_7] = (char) (i_9 + 87);
					} else {
						arr_6[i_7] = (char) (i_9 + 48);
					}
				}

				return new String(arr_6);
			} else {
				return Integer.toString(i_0, i_1);
			}
		} else {
			throw new IllegalArgumentException("" + i_1);
		}
	}

	static void method14663(int i_0, int i_1, byte b_2) {
		Class282_Sub50_Sub12 class282_sub50_sub12_3 = Engine.getIComponentVar(7, (long) i_0);
		class282_sub50_sub12_3.method14995(2079559306);
		class282_sub50_sub12_3.anInt9668 = i_1;
	}

}
